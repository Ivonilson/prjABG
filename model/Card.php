...
class Card {
	...
	public function cadItemCard()
	{
			...
			$statement = "INSERT INTO tbl_card (item, titulo, descricao, observacoes, data_cadastro, alteracoes, usuario) VALUES (:item, :titulo, :descricao, :observacoes, CURRENT_TIMESTAMP(), :alteracoes, :usuario)";
		...
		{
			$querySelect = "SELECT id, item, titulo, descricao, observacoes, usuario  FROM tbl_card WHERE item  = '$item'";
		...
		public function edItemCard()
		{
			$dados = filter_input_array(INPUT_POST, FILTER_DEFAULT);

			if (!empty($dados['btnEditarItemCard'])) {
				unset($dados['btnEditarItemCard']);
			}
			...
			$statement = "UPDATE tbl_card set titulo = :titulo, descricao = :descricao, observacoes = :observacoes, alteracoes = CONCAT(IFNULL(alteracoes, ''), :alteracoes) WHERE item = :item";
		...
		public function historicoItem($item){
			...
			$querySelect = "SELECT id, item, titulo, descricao, observacoes, data_cadastro, alteracoes, usuario  FROM tbl_card WHERE item  = '$item'";
